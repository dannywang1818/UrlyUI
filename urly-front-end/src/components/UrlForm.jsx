...
export default function UrlForm() {
  ...
  return (
    <div className="sm:flex sm:flex-col sm:rounded-lg px-4 py-5 sm:p-6 w-2/3">
        ...
        <div className="mt-5 sm:mt-0 sm:ml-6 sm:flex-shrink-0 sm:flex sm:items-center">
          <CopyToClipboard text={urls.length > 0 ? urls[0].short : ""}>
            <button
              ...
              className={
                buttonColor +
                "inline-flex items-center px-4 py-2 border border-transparent shadow-sm font-medium rounded-md text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:text-lg"
              }
            >
              {buttonText}
            </button>
      ...
      {urls != undefined && urls.length > 0 && <UrlList urls={urls} />}
    </div>
  );
}
